...
def _draw_table_line(c: canvas.Canvas, x, y, w):
...
def generate_pdf(data: QuoteData) -> bytes:
    ...
    if info.brand_name or info.code:
        if info.brand_name:
            _draw_text(c, MARGIN_L, y, f'店名：{info.brand_name}', bold=True, size=12,
                       max_width=half_w)
        if info.code:
            _draw_text(c, MARGIN_L + CONTENT_W * 0.52, y, f'代號：{info.code}', bold=True, size=12,
                       max_width=half_w)
        y -= line_h

    # Title (company name)
    _draw_text(c, MARGIN_L, y, f'抬頭：{info.title}', bold=True, size=12, max_width=CONTENT_W)
    y -= line_h

    # Contact + phone
    _draw_text(c, MARGIN_L, y, f'聯絡人：{info.contact_name}', bold=True, size=12,
               max_width=half_w)
    ...
    if phones:
        ...
        if not any(kw in first.upper() for kw in ['TEL', 'MOB', 'PHONE', '手機', '電話']):
            first = f'TEL : {first}'
        _draw_text(c, MARGIN_L + CONTENT_W * 0.52, y, first, bold=True, size=12,
                   max_width=half_w)
        for extra_phone in phones[1:]:
            y -= line_h
            _draw_text(c, MARGIN_L + CONTENT_W * 0.52, y, extra_phone, bold=True, size=12,
                       max_width=half_w)
    y -= line_h

    # Address + invoice address
    addr_label = '店鋪地址' if info.invoice_address else '地址'
    _draw_text(c, MARGIN_L, y, f'{addr_label}：{info.address}', bold=True, size=12,
               max_width=half_w)
    _draw_text(c, MARGIN_L + CONTENT_W * 0.52, y, f'統一編號：{info.tax_id}', bold=True, size=12,
               max_width=half_w)
    y -= line_h

    if info.invoice_address:
        _draw_text(c, MARGIN_L, y, f'發票地址：{info.invoice_address}', bold=True, size=12,
                   max_width=half_w)
        y -= line_h

    # Unit + date
    _draw_text(c, MARGIN_L, y, '單位：條 / 幣別：新台幣', bold=True, size=12, max_width=half_w)
    _draw_text(c, MARGIN_L + CONTENT_W * 0.52, y, f'日期：{info.date}', bold=True, size=12,
               max_width=half_w)
    y -= 24
    ...
    # ── Footer ──
    footer_y = MARGIN_B + 60
    if y < footer_y + 30:
        footer_y = y - 20

    c.setLineWidth(0.8)
    c.line(MARGIN_L, footer_y + 50, PAGE_W - MARGIN_R, footer_y + 50)
    _draw_text(c, MARGIN_L, footer_y + 34, '確認後請蓋章回傳，謝謝您。', bold=True, size=12)

    _draw_text(c, PAGE_W / 2, MARGIN_B + 10,
...
def _draw_new_product_table(c: canvas.Canvas, items, y) -> float:
    ...
    for j, (header, align) in enumerate(zip(headers, aligns)):
        ...
        if align == 'center':
            cx += col_w[j] / 2
            _draw_text(c, cx, y - 15, header, bold=True, size=12, align='center')
        else:
            _draw_text(c, cx + 6, y - 15, header, bold=True, size=12)
    y -= row_h
    ...
    for item in items:
        ...
        _draw_text(c, x0 + 6, y - 15, item.name, size=12, max_width=col_w[0] - 12)
        _draw_text(c, x0 + col_w[0] + col_w[1] / 2, y - 15, item.quantity, size=12, align='center')
        _draw_text(c, x0 + col_w[0] + col_w[1] + col_w[2] - 8, y - 15, _fmt(p), size=12, align='right')
        _draw_text(c, x0 + sum(col_w[:3]) + col_w[3] - 8, y - 15, _fmt(amount), size=12, align='right')
        y -= row_h

    # Tax row
    tax = subtotal * 0.05
    _draw_table_rect(c, x0, y, col_w, row_h)
    _draw_text(c, x0 + col_w[0] + col_w[1] + col_w[2] - 8, y - 15, '稅金 5%', bold=True, size=12, align='right')
    _draw_text(c, x0 + sum(col_w[:3]) + col_w[3] - 8, y - 15, _fmt(tax), size=12, align='right')
    y -= row_h

    # Total row
    total = subtotal + tax
    _draw_table_rect(c, x0, y, col_w, row_h)
    _draw_text(c, x0 + col_w[0] + col_w[1] + col_w[2] - 8, y - 15, '總計', bold=True, size=12, align='right')
    _draw_text(c, x0 + sum(col_w[:3]) + col_w[3] - 8, y - 15, _fmt(total), bold=True, size=12, align='right')
    y -= row_h
...
def _draw_rental_table(c: canvas.Canvas, items, y) -> float:
    ...
    for side, side_items in enumerate([left_items, right_items]):
        ...
        _draw_text(c, x0 + 6, cur_y - 15, '品項', bold=True, size=12)
        _draw_text(c, x0 + col_name_w + col_price_w / 2, cur_y - 15, '單價', bold=True, size=12, align='center')
        cur_y -= row_h

        for item in side_items:
            _draw_table_rect(c, x0, cur_y, [col_name_w, col_price_w], row_h)
            _draw_text(c, x0 + 6, cur_y - 15, item.name, size=12, max_width=col_name_w - 12)
            _draw_text(c, x0 + col_name_w + col_price_w - 8, cur_y - 15, item.price, size=12, align='right')
            cur_y -= row_h
